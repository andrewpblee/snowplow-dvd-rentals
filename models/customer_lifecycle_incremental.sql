...
with find_new_rentals as (
    -- the rental updates hourly, and we only need to bring in the information for the last hour
    -- however we need to see the whole life of a customer, so find customers that have a rental 
    -- in the last hour (or since the last run)
    select distinct
        customer_id 
    from {{ ref('rental') }}
    {% if is_incremental() %}
...
customer_info as (
  ...
  left join {{ ref('rental') }} as r
    on c.customer_id = r.customer_id
  left join {{ ref('payment') }} as p
    on r.rental_id = p.rental_id
  inner join find_new_rentals as fnr 
    on c.customer_id = fnr.customer_id
),
